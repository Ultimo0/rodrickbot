...
export default {
  ...
  description:
    'Résume un texte, un long message cité ou un document (.txt/.md/.csv/.json/.pdf/.docx). Usage: !resume [court|moyen|détaillé] <texte>, ou réponds à un message/document avec !resume [taille].',
  ...
  execute: async (ctx) => {
    ...
    const { text: inputText, truncated } = truncateText(trimmed, MAX_CHARS);
    ...
    try {
      ...
      await ctx.replyRaw({ text: prefix + summary });
      await ctx.success();
    } catch (err) {
